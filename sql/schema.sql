...
-- OAuth2 用户绑定表
-- ============================================================
create table if not exists ifdian_oauth2_bindings
(
    id               bigint auto_increment primary key comment '自增主键',
    user_id          varchar(64)  not null comment '爱发电用户 ID (公开)',
    user_private_id  varchar(64)  not null comment '爱发电用户唯一标识 (私有)',
    name             varchar(128) null comment '爱发电用户昵称',
    avatar           varchar(512) null comment '爱发电用户头像 URL',
    local_user_id    varchar(128) null comment '业务系统自定义用户标识',
    client_id        varchar(64)  null comment 'OAuth2 客户端 ID',
    access_token     varchar(256) null comment 'access_token',
    created_at       timestamp default current_timestamp comment '首次绑定时间',
    updated_at       timestamp default current_timestamp
                         on update current_timestamp comment '最近更新时间',

    constraint uk_user_private_id unique (user_private_id)
) engine = InnoDB
  default charset = utf8mb4
  collate = utf8mb4_unicode_ci
    comment 'OAuth2 用户绑定表';

-- 索引
create index idx_oauth2_user_id       on ifdian_oauth2_bindings (user_id);
create index idx_oauth2_local_user_id on ifdian_oauth2_bindings (local_user_id);
